...
- **Update the implementation plan (if applicable):** If this bug fix was tracked as a task in an implementation plan in `plans/`, identify the relevant plan (check active plans in `plans/` for a task matching this fix), update that plan: mark the completed task with `- [✅]` (green check mark) and add a brief note if needed. When all tasks in the plan are complete, add a completion marker (e.g. `**Status:** ✅ COMPLETED`) per the Workflow-Scripts main README, "Completion Status Conventions."
