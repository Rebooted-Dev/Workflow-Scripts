...
tools/wf route "review this plan" >/dev/null
...
WF_ROOT="$tmpdir" tools/wf validate >/tmp/wf-valid.out
WF_ROOT="$tmpdir" tools/wf build >/tmp/wf-build.out
WF_ROOT="$tmpdir" tools/wf build --check >/tmp/wf-build-check.out
WF_ROOT="$tmpdir" tools/wf route "review this plan" | grep -q '"id": "plan-review"'
...
if WF_ROOT="$tmpdir" tools/wf validate >/tmp/wf-duplicate.out 2>&1; then
...
if WF_ROOT="$tmpdir" tools/wf validate >/tmp/wf-invalid-kind.out 2>&1; then
...
WF_ROOT="$tmpdir" tools/wf validate >/tmp/wf-warnings.out
...
if WF_ROOT="$tmpdir" tools/wf validate >/tmp/wf-bad-role.out 2>&1; then
...
if WF_ROOT="$tmpdir" tools/wf validate >/tmp/wf-core-dup.out 2>&1; then
...
# Second Valid Record
MD
WF_ROOT="$tmpdir" tools/wf build >/tmp/wf-build-2.out
printf '\nmanual edit\n' >>"$tmpdir/ROUTER.md"
if WF_ROOT="$tmpdir" tools/wf build --check >/tmp/wf-stale-build.out 2>&1; then
...
WF_ROOT="$bookdir" tools/wf build >/tmp/wf-book-build.out
WF_ROOT="$bookdir" tools/wf log added "Bookkeeping Command" --notes "created by fixture" >/tmp/wf-book-log.out
...
WF_ROOT="$bookdir" tools/wf trouble validation fixture-issue --title "Fixture Issue" --status resolved >/tmp/wf-book-trouble.out
...
WF_ROOT="$bookdir" tools/wf new plan fixture-plan --title "Fixture Plan" >/tmp/wf-book-plan.out
...
Finished.
MD
WF_ROOT="$bookdir" tools/wf file-completed "00-project/plans/demo/$(date +%F)-done.md" review >/tmp/wf-book-completed.out
...
if WF_ROOT="$bookdir" tools/wf file-completed "00-project/plans/missing.md" review >/tmp/wf-book-missing.out 2>&1; then
  echo "expected missing completed-plan source to fail" >&2
  exit 1
fi
grep -q "source plan does not exist" /tmp/wf-book-missing.out

WF_ROOT="$bookdir" tools/wf debt add architecture "Fixture Debt" --notes "defer cleanup" >/tmp/wf-book-debt.out
test -f "$bookdir/00-project/debt/architecture/$(date +%F)-architecture-fixture-debt.md"
WF_ROOT="$bookdir" tools/wf debt list --status open | grep -q "Fixture Debt"
...
# Run Plan
MD
WF_ROOT="$bookdir" tools/wf run plan-review 00-project/plans/demo/run-plan.md --output 00-project/plans/demo/review.md --timeout 1 >/tmp/wf-run.out
...
WF_ROOT="$bookdir" tools/wf stats runs 00-project/plans/demo | grep -q '"runs": 1'

cat >"$bookdir/11-Skills/example/SKILL.md" <<'MD'
# Example Skill
MD
WF_ROOT="$bookdir" tools/wf build skills >/tmp/wf-skills.out
test -f "$bookdir/dist/skills/codex/example/SKILL.md"
WF_ROOT="$bookdir" tools/wf build skills --check >/tmp/wf-skills-check.out
