...
- **When to create changelog entries**: For any change to consolidation artifacts, Workflow-Scripts migration work, or `00-project/` meta (features, fixes, refactors, docs, config). One file per change. For **completed plans**, default is **`plans-completed/<category>/`** plus a Type=`plan` row in **`changelog/index.md`**; alternate is **`changelog/plans/`** when explicitly requested.
- **Location**: Use the `changelog/` directory. Do NOT use a single `CHANGELOG.md` at workspace root.
- **Structure**:
...
- **Structure**:
  - Create individual files in the appropriate category folder (`build/`, `runtime/`, `data/`, `environment/`, `security/`, `git/`)
...
**Note**: "Logs" refer to `changelog/` and `troubleshooting/`, not application `.log` files or the sibling `Drag-Free-v2/logs/` execution transcripts.
...
- **Plans**: `plans/README.md` maps the project dir. `plans/TODO.md` holds current tasks. Active plan documents live in `plans/` by default; use `build/` for active plans only when the project map explicitly says so.
