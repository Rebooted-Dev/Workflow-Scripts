...
## Purpose
Perform a structured analysis to identify performance bottlenecks, resource inefficiencies, and optimization opportunities, then file a report in `<metadata-root>/research/` using the metadata-root rule in `../../core/meta/naming-conventions.md`.
...
Use `../../core/standards/code-design.md`, `../../core/standards/error-handling.md`, `../../core/standards/observability.md`, and `../../core/standards/security-baseline.md` as the shared quality baseline when optimization findings affect design, failure behavior, runtime signals, or security posture.
...
- Memory/resource usage, leaks, pooling, and bounded growth per `../../core/standards/code-design.md`.
- Bundle size, lazy loading, and code splitting.
- Rendering performance for UI code.
- Concurrent operations, race conditions, and fallback/retry behavior per `../../core/standards/error-handling.md`.
- Runtime signals needed to prove the optimization worked per `../../core/standards/observability.md`.
