...
| W6 | **Numbering/taxonomy confusion** | Three unrelated `00-` roots (`00-Meta-Workflow/`, `00-project/`, `00-project-setup/`); `09` missing; `07/08/10/12` are reference guides not workflows; `04-documentation/` contains `00/01/02/03/09`; non-workflow prompt files sit inside workflow categories (for example, `04-documentation/ascii-art-prompts.md`) | Discovery cost, misleading sequence semantics |
