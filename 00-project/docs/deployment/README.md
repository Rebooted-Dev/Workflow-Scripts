...
Or manual `git commit` + `git push`.

## Multi-Project Sync

For maintainers with multiple host projects:

1. Configure `PROJECTS` array in `scripts/sync-workflow-scripts.sh`
2. Run:

```bash
./scripts/sync-workflow-scripts.sh --status    # check state
./scripts/sync-workflow-scripts.sh --dry-run   # preview
./scripts/sync-workflow-scripts.sh --auto      # pull all
```
...
| `v1.5`, `v1.6` | Versioned release lines |
