...
Paths below are relative to `workflows-drag-free/00-project/` unless noted.

---

## Changelog System (`changelog/` directory)

- **When to create changelog entries**: For any change to workflows-drag-free content or this `00-project/` meta (features, fixes, refactors, docs, config). One file per change. For **completed plans**, default is **`plans-completed/<category>/`** plus a Type=`plan` row in **`changelog/index.md`**; alternate is **`changelog/plans/`** when explicitly requested.
- **Location**: Use the `changelog/` directory. Do NOT use a single `CHANGELOG.md` at tree root.
- **Structure**:
...
- **Docs**: `docs/` holds project documentation; `docs/agents/` holds agent-facing detailed guides. Slim `AGENTS.md` links to `docs/agents/`.
