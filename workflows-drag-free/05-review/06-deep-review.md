...
- Use `engineering-quality-gates` to keep findings tied to explicit quality standards.
- Use `delegated-agent-orchestration` when splitting review work across multiple lenses or models.
- Use `repo-records-and-filing` to file the report, verification addendum, and follow-up plan in the correct metadata root.
