...
triggers:
  - "fix this bug"
  - "debug this issue"
inputs: [bug-report, repository-root]
outputs: [bug-fix, regression-test, changelog-entry]
requires: [metadata-root, filing-and-logging, verification-gates, code-design, error-handling, observability, security-baseline]
agents: [bug-hunter, test-writer, security-scanner, docs-writer]
prev: [bug-description]
next: [confirm-execution]
skills:
  primary: workflow-bug-fix-plan-and-logs
  required:
  ...
  conditional:
    - skill: delegated-agent-orchestration
      when: "root-cause search, regression testing, or security impact requires helper agents"
---
...
- Use primary skill `workflow-bug-fix-plan-and-logs` for diagnosis, fix planning, regression testing, and log updates.
...
- Load conditional skill `delegated-agent-orchestration` when splitting investigation or verification across helpers.
...
**Agent Spawning Policy:** Follow `../../core/meta/agent-spawning-policy.md`: use 3-6 total agents, start with 2-3 core roles, add triggered specialist roles only when evidence justifies them, and split into sessions if more roles are needed.
...
- **Update the implementation plan (if applicable):** For task marking, completion markers, and archiving completed plans, follow the single source of truth: **[`../documentation/03-mark-completed.md`](../documentation/03-mark-completed.md)**.
...
- **Agent file instruction:** Ensure the project's agent files (AGENTS.md, and optionally CLAUDE.md, GEMINI.md) include the line **Bugs: add regression test when it fits.** so that all coding agents (Codex, Cursor, Claude, etc.) add regression tests when fixing bugs. If the line is missing, add it to AGENTS.md (and optionally to CLAUDE.md, GEMINI.md) as part of the fix or in a follow-up. See [00-project-setup/01-setup-project.md](../setup/01-setup-project.md) Step 1.3 for setup instructions.
...
- **[`../build/01-execution.md`](../build/01-execution.md)** - Implement bug fixes using execution workflow
- **[`../review/01-code-review.md`](../review/01-code-review.md)** - Review bug fixes before merging
- **[`../planning/02-finalise-plan.md`](../planning/02-finalise-plan.md)** - Create implementation plan for complex bug fixes
- **[`../security/02-security-fix.md`](../security/02-security-fix.md)** - Fix security-related bugs
