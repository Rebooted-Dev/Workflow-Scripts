...
## Inputs
- Repository root
- Plan or report files that declare completed tasks (usually under `project/plans/`; also include `project/build/*.md` only when the project map uses that active-plan location, and `project/changelog/plans/*.md` only for explicit archive checks)
...
1. **Locate plan and report files** that declare completed tasks:
   - Scan `project/plans/` by default for markdown files with checkboxes, "Complete", "✅" (green check mark), or "Implementation Verified"; include `project/build/` only when the project map uses it for active plans, and `project/changelog/plans/` only when validating that explicit archive
...
- **[`../../core/meta/severity-priority-rubric.md`](../../core/meta/severity-priority-rubric.md)** — Severity/priority for ordering flagged issues
- **[`../planning/02-finalise-plan.md`](../planning/02-finalise-plan.md)** — Finalise plans before marking completed
