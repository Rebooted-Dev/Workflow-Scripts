...
**Status:** Implemented (Phases 1–4 core complete via Drag-Free-v2 unified plan, 2026-07-06) — follow-up items remain
**Implementation record:** `Workflow-Scripts-consolidated/working-tree-files/00-project/plans/Drag-Free-v2/2026-07-06-drag-free-v2-unified-implementation-plan.md`
...
- [✅] Add frontmatter to all workflow files in `00-…`–`06-…` + `11-Skills` (Effort: M) — 139 frontmatter records at Phase 6
...
- [ ] Fix all broken links/contradictions surfaced by first `wf validate` run — including the known `00-research-and-plan.md` output-location contradiction and `../02-build-code/` dead links (Effort: S) — **55 warnings remain** (`Workflow-Scripts-consolidated/logs/README.md`)
...
- [✅] Trim workflows to reference partials per KI-10: delete reference-then-restate copies (survey table §A lists the line ranges), strip "When to Use"/"Related Workflows" navigation (router replaces it), externalize embedded output templates and example pairs to `templates/`; record per-file before/after token counts (Effort: M) — pilot files measured; median 30.5% reduction (below §8 target)
...
- [✅] Replace per-workflow role menus with role-ID references; extend frontmatter `agents: {min, max}` to `agents: {min, max, core: […], triggered: […]}` and validate role IDs against the registry (Effort: S) — key planning/build/review/security workflows; some review/debug files still have free-text spawn menus
...
- [✅] Port `orchestrator-review.sh` focus areas + status JSON into the manifest contract; deprecate the script (Effort: S) — wrapper retained at `tools/orchestrator/orchestrator-review.sh`
- [ ] Add `--fanout <profile>` delegating to the 2026-07-03 plan's profiles once piloted (Effort: M)
- [✅] Implement `wf build skills` for claude + codex targets; fold `11-Skills/` sources into parent workflows (Effort: L) — bundles generated under `dist/skills/`; `11-Skills/` hand copies not yet retired
- [✅] Implement `wf stats` over run manifests (Effort: S)
- [✅] Teach `wf run` to read role registry hints: `tier` selects launch model class, `verify: different-model` schedules the verification pass, role IDs recorded in `_manifest.json`; `wf stats` gains per-role breakdowns (Effort: S) — smoke run used stub `opencode`; production provider run pending
...
- [✅] Merge `00-Meta-Workflow/00-meta` into `core/`; retire `00-Meta-Workflow/` (00-docs → `00-project/research/archive/`) (Effort: S) — merged to `core/meta/`; redirect stubs remain for one release cycle
...
- [ ] `wf validate` green in CI; zero broken cross-references in live docs (measured, not asserted) — 55 warnings remain; no Workflow-Scripts CI gate yet
- [✅] Every filing/logging ritual has a one-command form; filing a completed plan takes one verb, not four edits.
- [✅] Each convention/rule exists in exactly one source file (grep-verifiable for the marker phrases currently duplicated) — duplication linter in `tools/wf validate`
- [ ] One workflow source produces conforming Claude and Codex skill bundles; `11-Skills/` hand-copies retired — bundles generated; hand-authored `11-Skills/` retained pending parity review
- [✅] Every delegated run — single or fan-out — leaves a `_manifest.json`; `wf stats` renders an accurate run ledger.
- [ ] Median workflow file token count reduced ≥ 40% overall and ≥ 55% in the review/security/planning families (partials extraction + KI-10 authoring standard), with no loss of standalone readability; per-file before/after counts recorded — achieved 30.5% median on pilot files
- [ ] Every sub-agent role referenced in any workflow resolves to a `core/roles/` registry entry with output contract, evidence standard, tool scope, and tier hint; zero free-text "spawn 1 X agent" menus remain (grep-verifiable) — pilot workflows converted; refactoring/debug/docs files retain legacy menus
- [ ] Run manifests record role IDs; `wf stats` reports confirmed-vs-noise finding rates per role/model combination — role IDs recorded; confirmed-vs-noise rates need real provider runs
...
- [✅] This proposal was reconciled into `2026-07-06-drag-free-v2-unified-implementation-plan.md` and reviewed/finalised before implementation (Phase 0).
- [✅] TODO row added in `00-project/plans/TODO.md` (Drag-Free-v2 follow-up items tracked separately).
- Remaining follow-up: fix `tools/wf validate` warnings, add Workflow-Scripts CI gate, promote consolidated archive to live repo.
...
- **2026-07-06 (implementation status sync):** Checkboxes updated to reflect Drag-Free-v2 Phases 1–6 execution. Completed items marked `- [✅]`; open items retain `- [ ]` with notes where targets were partially met.
