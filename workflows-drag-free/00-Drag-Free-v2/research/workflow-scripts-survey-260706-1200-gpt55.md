...
>
> **Implementation status (2026-07-06):** Baseline survey of the pre-v2 tree. Drag-Free-v2 Phases 1–6 implemented the recommendations below (frontmatter, `tools/wf`, `core/` partials, role registry, directory rationalization). Open items: 55 `wf validate` warnings, Workflow-Scripts CI gate, full link cleanup, `11-Skills/` retirement.
...
1. [✅] Normalize artifact routing across planning/debugging/build docs to the metadata-root rule in `00-Meta-Workflow/00-meta/naming-conventions.md` — `core/metadata-root.md`
...
4. [✅] Add a validator for stale path names:
   - Catch `02-build-code`, root `plans/`, root `plans-completed/`, old `changelog/plans` defaults, and inconsistent `project/build` references. — `tools/wf validate` (55 warnings remain to fix)

5. [ ] Move or clearly label non-workflow prompt/reference files:
   - `01-planning-and-organizing/fable-like.md`
   - `04-documentation/ascii-art-prompts.md`

6. [✅] Make `04-documentation/03-mark-completed.md` the single canonical completion/filing authority and have build/debug/planning workflows link to it instead of restating filing rules — `core/filing-and-logging.md` + `wf file-completed`

7. [✅] Add a compact workflow-to-skill mapping table so agents know when to use the short `11-Skills/*` version versus the long workflow file — `skills:` frontmatter + skill layer plan; `catalog.json` skill mappings pending `tools/wf build` regeneration in live repo
